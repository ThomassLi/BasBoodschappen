	...
	public function dropDownKlant($row_selected = -1){
		...
		foreach ($lijst as $row){
			if($row_selected == $row["klantId"]){
				echo "<option value='$row[klantId]' selected='selected'> $row[klantNaam] $row[klantEmail] $row[klantAdres] $row[klantPostcode] $row[klantWoonplaats]</option>\n";
			} else {
				echo "<option value='$row[klantId]'> $row[klantNaam] $row[klantEmail] $row[klantAdres] $row[klantPostcode] $row[klantWoonplaats]</option>\n";
			}
		}
		echo "</select>";
	}
